...
export async function POST(request: NextRequest) {
  try {
      ...
      if (newEventResponse.ok) {
        ...
        await prisma.reminder.create({
          data: {
            ...
            status: 'pending',
            snoozeCount: updatedDbReminder.snoozeCount,
          },
        })
      } else {
    ...
    if (!updateGoogleEventResponse.ok) {
      console.warn(
        'Failed to update Google Calendar event description:',
        await updateGoogleEventResponse.text()
      )
    }
    ...
    return NextResponse.json({
      ...
      message:
        status === 'missed'
          ? `Reminder rescheduled for ${snoozeDuration}`
          : 'Status updated successfully',
    })
  } catch (error) {
    console.error('Error updating reminder:', error)
    return NextResponse.json(
      { error: (error as Error).message || 'Failed to update reminder' },
      { status: 500 }
    )
  }
}
